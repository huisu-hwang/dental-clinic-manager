...
> ⚠️ **Deprecated (2026-04)** — 이 standalone 에이전트는 더 이상 신규 설치를 권장하지 않습니다.
> 덴트웹 동기화 기능은 **통합 워커**(Electron 데스크탑 앱, `marketing-worker/electron/`)에
> 내장되었습니다. 신규 설치는 대시보드의 "통합 워커 다운로드" 버튼을 사용해주세요.
> 통합 워커를 설치하면 마케팅 발행, 홈택스 스크래핑, SEO 분석, 이메일 모니터링,
> **덴트웹 동기화**가 하나의 프로그램으로 동작합니다.
>
> 이 디렉토리는 기존 설치본 호환성을 위해 유지되며, 소스 코드 참조용으로만 사용됩니다.
