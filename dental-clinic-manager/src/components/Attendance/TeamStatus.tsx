...
export default function TeamStatus() {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'leave':
        return (
          <svg className="w-5 h-5 text-at-accent" fill="currentColor" viewBox="0 0 20 20">
            <path
              fillRule="evenodd"
      ...
      default:
        return (
          <svg className="w-5 h-5 text-at-text-weak" fill="currentColor" viewBox="0 0 20 20">
            <path
              fillRule="evenodd"
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-at-text">팀 출퇴근 현황</h1>
        ...
        <div className="flex items-center gap-3">
          <label className="flex items-center text-sm text-at-text-secondary">
            <input
              type="checkbox"
              checked={autoRefresh}
              onChange={(e) => setAutoRefresh(e.target.checked)}
              className="w-4 h-4 text-at-accent border-at-border rounded focus:ring-at-accent mr-2"
            />
            자동 새로고침
          </label>
          <button
      ...
      <div className="bg-white rounded-2xl shadow p-6">
        <div className="flex items-center gap-4">
          <label className="text-sm font-medium text-at-text-secondary">날짜 선택</label>
          <input
            type="date"
            value={selectedDate}
            onChange={(e) => setSelectedDate(e.target.value)}
            className="px-3 py-2 border border-at-border rounded-xl focus:ring-2 focus:ring-at-accent focus:border-transparent"
          />
          {isToday && (
            <span className="px-3 py-1 bg-at-success-bg text-green-800 text-sm font-medium rounded-full">
              오늘
            </span>
          )}
        </div>
      ...
      {loading ? (
        <div className="bg-white rounded-2xl shadow p-12 text-center">
          <div className="w-12 h-12 border-4 border-at-accent border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
          <p className="text-at-text-secondary">팀 현황 로딩 중...</p>
        </div>
      ) : teamStatus ? (
        <>
          {/* 요약 통계 */}
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
            <div className="bg-white rounded-xl shadow p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-at-text">{teamStatus.total_employees}명</p>
                </div>
                <div className="w-10 h-10 bg-at-surface-alt rounded-full flex items-center justify-center">
                  <svg className="w-5 h-5 text-at-text-secondary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="bg-white rounded-xl shadow p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-at-text-secondary">출근</p>
                  <p className="text-2xl font-bold text-at-success">{teamStatus.checked_in}명</p>
                </div>
                <div className="w-10 h-10 bg-at-success-bg rounded-full flex items-center justify-center">
                  <svg className="w-5 h-5 text-at-success" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="bg-white rounded-xl shadow p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-at-text-secondary">퇴근 완료</p>
                  <p className="text-2xl font-bold text-at-accent">{teamStatus.checked_out || 0}명</p>
                </div>
                <div className="w-10 h-10 bg-at-tag rounded-full flex items-center justify-center">
                  <svg className="w-5 h-5 text-at-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="bg-white rounded-xl shadow p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-at-text-secondary">미출근</p>
            ...
            <div className="bg-white rounded-xl shadow p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-at-text-secondary">지각</p>
                  <p className="text-2xl font-bold text-at-warning">{teamStatus.late_count}명</p>
                </div>
                <div className="w-10 h-10 bg-yellow-100 rounded-full flex items-center justify-center">
                  <svg className="w-5 h-5 text-at-warning" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="bg-white rounded-xl shadow p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-at-text-secondary">조퇴</p>
                  <p className="text-2xl font-bold text-at-error">{teamStatus.early_leave_count || 0}명</p>
                </div>
                <div className="w-10 h-10 bg-at-error-bg rounded-full flex items-center justify-center">
                  <svg className="w-5 h-5 text-at-error" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="bg-white rounded-xl shadow p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-at-text-secondary">초과근무</p>
          ...
          <div className="bg-white rounded-xl shadow overflow-hidden">
            <div className="px-6 py-4 border-b border-at-border">
              <h2 className="text-lg font-semibold text-at-text">직원별 출퇴근 현황</h2>
            </div>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-at-border">
                <thead className="bg-at-surface-alt">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-medium text-at-text-weak uppercase tracking-wider">
                      상태
                    </th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-at-text-weak uppercase tracking-wider">
                      이름
                ...
                <tbody className="bg-white divide-y divide-at-border">
                  {teamStatus.employees.map((employee) => (
                    <tr key={employee.user_id} className="hover:bg-at-surface-alt">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-sm text-at-text">
                        {formatTime(employee.check_in_time)}
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap text-sm text-at-text-weak">
                          ...
                          return totalOT > 0 ? (
                            <div>
                              ...
                              {mealMinutes > 0 && employee.overtime_minutes > 0 && (
                                <div className="text-xs text-at-text-weak">{employee.overtime_minutes}+{mealMinutes}</div>
                              )}
                            </div>
                          ) : (
                            <span className="text-at-text-weak">-</span>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-sm">
                        {employee.total_work_minutes && employee.total_work_minutes > 0 ? (
                          <span className="text-at-accent font-medium">
        ...
        <div className="bg-white rounded-xl shadow p-12 text-center">
          <svg className="w-16 h-16 text-at-text-weak mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
          </svg>
          <p className="text-at-text-secondary">팀 출근 현황을 불러올 수 없습니다.</p>
        </div>
      )}
