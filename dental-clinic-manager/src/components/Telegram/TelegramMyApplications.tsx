  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-at-text-secondary">내 신청 현황</h3>
      </div>
      ...
      <div className="space-y-2">
        {applications.map(app => {
          ...
          return (
            <div
              key={app.id}
              className="flex items-start justify-between p-3 rounded-xl border border-at-border bg-white"
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <h4 className="text-sm font-medium text-at-text truncate">{app.board_title}</h4>
                ...
                {app.status === 'rejected' && app.rejection_reason && (
                  <p className="text-xs text-at-error mt-1 bg-at-error-bg px-2 py-1 rounded-lg">
                    반려 사유: {app.rejection_reason}
                  </p>
                )}
              </div>
              <span className="text-[10px] text-at-text-weak ml-2 flex-shrink-0">
