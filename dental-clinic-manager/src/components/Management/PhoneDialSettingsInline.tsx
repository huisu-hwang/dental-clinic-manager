  ...
  const handleDetectNetwork = async () => {
    ...
    try {
      ...
      if (info.localIP) {
        ...
        setTestResult({
          success: false,
          message: '네트워크 정보를 감지할 수 없습니다. 브라우저 설정을 확인해주세요.'
        })
      }
    } catch (error) {
      setTestResult({
  ...
  const handleScanDevices = async () => {
    ...
    try {
      ...
      if (found.length > 0) {
        setTestResult({
          success: true,
          message: `${found.length}개의 기기를 발견했습니다. 아래 목록에서 선택하세요.`
        })
      } else {
        setTestResult({
          success: false,
          message: '응답하는 기기를 찾지 못했습니다. IP 주소를 직접 입력해주세요.'
        })
      }
    } catch (error) {
      setTestResult({
          ...
          <div className="p-4 bg-white rounded-lg border border-slate-200">
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              ...
              >
                ...
                {detectingNetwork ? '감지 중...' : '내 네트워크 감지'}
              </button>
              ...
              {networkInfo?.localIP && (
                <button
                  ...
                  disabled={scanning}
                  className="flex items-center gap-2 px-3 py-2 bg-green-600 hover:bg-green-700 disabled:bg-green-400 text-white text-sm rounded-lg transition-colors"
                >
                  {scanning ? (
                    <Loader2 className="w-4 h-4 animate-spin" />
                  ) : (
            ...
            {networkInfo?.localIP && (
              <div className="text-xs text-slate-500 mb-2">
                내 IP: <span className="font-mono text-slate-700">{networkInfo.localIP}</span>
                {networkInfo.subnet && (
                  <span> (검색 범위: {networkInfo.subnet}.x)</span>
                )}
              </div>
            )}
            ...
            {foundDevices.length > 0 && (
              <div className="mt-3">
                <div className="text-xs text-slate-600 mb-2">발견된 기기 (클릭하여 선택):</div>
            ...
            {networkInfo?.suggestedIPs && networkInfo.suggestedIPs.length > 0 && !scanning && foundDevices.length === 0 && (
              <div className="mt-3">
                <div className="text-xs text-slate-600 mb-2">일반적인 IP 주소 (클릭하여 입력):</div>
                <div className="flex flex-wrap gap-2">
                  {networkInfo.suggestedIPs.slice(0, 6).map((ip, index) => (
                    <button
                      key={index}
                      onClick={() => selectDevice(ip)}
                      className="px-2 py-1 text-xs rounded border border-slate-300 text-slate-600 hover:border-blue-400 hover:text-blue-600 transition-colors"
                    >
                      {ip}
                    </button>
                  ))}
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="text-sm text-blue-700">
            ...
            <ul className="list-disc list-inside space-y-1 text-blue-600">
              ...
              <li><strong>IP 전화기:</strong> 네트워크 감지 → 기기 검색으로 자동 설정</li>
            </ul>
          </div>
        </div>
      </div>
