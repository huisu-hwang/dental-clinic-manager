...
export default function SystemMonitoring() {
  ...
  const getStatusColor = (status: 'healthy' | 'warning' | 'error') => {
    switch (status) {
      case 'healthy': return 'text-at-success bg-at-success-bg'
      case 'warning': return 'text-at-warning bg-yellow-100'
      case 'error': return 'text-at-error bg-at-error-bg'
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-at-text">시스템 모니터링</h2>
        <button
          onClick={fetchSystemMetrics}
          disabled={refreshing}
          className="flex items-center space-x-2 px-4 py-2 bg-at-accent text-white rounded-md hover:bg-at-accent-hover disabled:opacity-50"
        >
      ...
      {loading ? (
        <div className="bg-white p-8 rounded-xl shadow-at-card border border-at-border">
          <div className="text-center">
            ...
            <p className="text-at-text-secondary">메트릭을 불러오는 중...</p>
          </div>
        </div>
      ) : (
        <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-white p-6 rounded-xl shadow-at-card border border-at-border">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-semibold text-at-text">데이터베이스</h3>
                  <CircleStackIcon className="h-5 w-5 text-at-text-weak" />
                </div>
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-at-text-secondary">상태</span>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-at-text-secondary">총 레코드</span>
                    <span className="text-sm font-medium">{metrics.database.totalRecords.toLocaleString()}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-at-text-secondary">저장 용량</span>
              ...
              <div className="bg-white p-6 rounded-xl shadow-at-card border border-at-border">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-semibold text-at-text">사용자 활동</h3>
                  <ArrowTrendingUpIcon className="h-5 w-5 text-at-text-weak" />
                </div>
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-at-text-secondary">일일 활성</span>
                    <span className="text-sm font-medium">{metrics.activity.dailyActiveUsers}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-at-text-secondary">주간 활성</span>
                    <span className="text-sm font-medium">{metrics.activity.weeklyActiveUsers}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-at-text-secondary">월간 활성</span>
              ...
              <div className="bg-white p-6 rounded-xl shadow-at-card border border-at-border">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-semibold text-at-text">성능</h3>
                  <ServerIcon className="h-5 w-5 text-at-text-weak" />
                </div>
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-at-text-secondary">응답 시간</span>
                    <span className="text-sm font-medium">{metrics.performance.avgResponseTime.toFixed(0)}ms</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-at-text-secondary">오류율</span>
                    <span className="text-sm font-medium">{metrics.performance.errorRate.toFixed(2)}%</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-at-text-secondary">가동률</span>
          ...
          {selectedTab === 'database' && (
            <div className="bg-white p-6 rounded-xl shadow-at-card border border-at-border">
              <h3 className="font-semibold text-at-text mb-4">데이터베이스 상태</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h4 className="text-sm font-medium text-at-text-secondary mb-3">테이블별 레코드 수</h4>
                  <div className="space-y-2">
                    {['users', 'clinics', 'daily_reports', 'consult_logs', 'gift_logs'].map(table => (
                      <div key={table} className="flex items-center justify-between p-2 bg-at-surface-alt rounded">
                        <span className="text-sm text-at-text-secondary">{table}</span>
                ...
                <div>
                  <h4 className="text-sm font-medium text-at-text-secondary mb-3">저장소 정보</h4>
          ...
          {selectedTab === 'activity' && (
            <div className="bg-white p-6 rounded-xl shadow-at-card border border-at-border">
              <h3 className="font-semibold text-at-text mb-4">사용자 활동</h3>
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="text-center p-4 bg-at-accent-light rounded-xl">
                    <div className="text-2xl font-bold text-at-accent">{metrics.activity.dailyActiveUsers}</div>
                    <div className="text-sm text-at-text-secondary">일일 활성 사용자</div>
                  </div>
                  <div className="text-center p-4 bg-at-success-bg rounded-xl">
                    <div className="text-2xl font-bold text-at-success">{metrics.activity.weeklyActiveUsers}</div>
                    <div className="text-sm text-at-text-secondary">주간 활성 사용자</div>
                  </div>
                  <div className="text-center p-4 bg-purple-50 rounded-xl">
                    <div className="text-2xl font-bold text-purple-600">{metrics.activity.monthlyActiveUsers}</div>
                    <div className="text-sm text-at-text-secondary">월간 활성 사용자</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          {selectedTab === 'alerts' && (
            <div className="bg-white p-6 rounded-xl shadow-at-card border border-at-border">
              <h3 className="font-semibold text-at-text mb-4">시스템 알림</h3>
              <div className="space-y-3">
                {metrics.alerts.length === 0 ? (
                  <p className="text-at-text-weak text-center py-8">현재 알림이 없습니다.</p>
                ) : (
                    ...
                    <div key={alert.id} className={`p-4 rounded-xl border ${
                      ...
                      <div className="flex items-start">
                        <ExclamationTriangleIcon className={`h-5 w-5 mr-3 mt-0.5 ${
                          alert.type === 'error' ? 'text-at-error' :
                          alert.type === 'warning' ? 'text-at-warning' :
                          'text-at-accent'
                        }`} />
                        ...
                        {!alert.resolved && (
                          <button className="ml-3 text-sm text-at-text-secondary hover:text-at-text">
                            해결됨으로 표시
                          </button>
                        )}
                      </div>
