...
const SectionHeader = ({ number, title, icon: Icon }: { number: number; title: string; icon: React.ElementType }) => (
  <div className="flex items-center space-x-3 pb-3 mb-4 border-b border-border">
    <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-brand-tint text-brand">
      <Icon className="w-4 h-4" />
    </div>
    <h3 className="text-base font-semibold text-text-primary">
      <span className="text-brand mr-1">{number}.</span>
      {title}
    </h3>
  </div>
)
...
export default function GuideSection() {
  return (
    <div>
      ...
      <div className="sticky top-14 z-10 bg-brand px-6 py-4 rounded-t-xl shadow-[var(--shadow-card)]">
        <div className="flex items-center space-x-3">
          <div className="w-10 h-10 bg-white/20 rounded-lg flex items-center justify-center">
            <BookOpen className="w-5 h-5 text-primary-foreground" />
          </div>
          <div>
            <h2 className="text-lg font-bold text-primary-foreground">사용 안내</h2>
      ...
      {/* 콘텐츠 영역 */}
      <div className="bg-card border-x border-b border-border rounded-b-xl p-6 space-y-8">
        {/* 최초 설정 */}
        <div>
          ...
          <ul className="space-y-3 text-slate-700">
            <li className="flex items-start">
              ...
              <span>
                <strong className="text-destructive">가장 먼저, Supabase 데이터베이스 테이블을 생성해야 합니다.</strong>{' '}
                이 작업은 최초 한 번만 필요합니다.
              </span>
            </li>
            <li className="flex items-start">
              ...
              <span>
                프로젝트 루트의 <code className="bg-surface-subtle px-2 py-0.5 rounded text-sm">supabase-schema.sql</code> 파일의 내용을 Supabase SQL Editor에서 실행하여 테이블을 생성하세요.
              </span>
            </li>
            <li className="flex items-start">
              <span className="text-text-tertiary mr-2">•</span>
              <span>
                <code className="bg-surface-subtle px-2 py-0.5 rounded text-sm">.env.local</code> 파일에 Supabase 프로젝트의 <strong>URL</strong>과 <strong>anon public key</strong>를 설정하세요.
              </span>
            </li>
          </ul>
        </div>
