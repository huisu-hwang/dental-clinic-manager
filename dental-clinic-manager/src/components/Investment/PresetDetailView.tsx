...
/**
 * 프리셋 전략 상세 뷰 — 페이지(/investment/strategy/preset/[id])와
 * 비교 페이지의 모달 양쪽에서 공통으로 사용.
 */
...
interface Props {
  presetId: string
  /** 모달 모드: true면 CTA 링크 숨김, 외부 컨테이너는 기본 패딩 없이 표시 */
  variant?: 'page' | 'modal'
}

export default function PresetDetailView({ presetId, variant = 'page' }: Props) {
  ...
  if (!preset) {
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-at-border p-8 text-center">
        ...
        <p className="text-sm text-at-text-secondary mt-1">ID: {presetId}</p>
      </div>
    )
  }
