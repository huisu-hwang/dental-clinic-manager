...
import {
  ...
  ArrowRight, Plus, Play, Pause, Trash2, BarChart3, Edit3,
} from 'lucide-react'
import Link from 'next/link'
import type { InvestmentStrategy, BrokerCredentialSafe } from '@/types/investment'
...
export default function InvestmentTab() {
  ...
  return (
    <div>
      {/* 서브메뉴 탭 (상단 고정) */}
      <div className="sticky top-14 z-10 bg-at-surface border-b border-at-border">
      ...
      <div className="p-4 sm:p-6">
        ...
        {subTab === 'connect' && (
          <div className="text-center py-8">
            <Link2 className="w-12 h-12 mx-auto mb-3 text-at-text-weak opacity-30" />
            <p className="text-sm text-at-text-secondary mb-3">계좌 연결 페이지로 이동합니다</p>
            <Link
        ...
        {subTab === 'trading' && (
          <div className="text-center py-8">
            <TrendingUp className="w-12 h-12 mx-auto mb-3 text-at-text-weak opacity-30" />
            <p className="text-sm text-at-text-secondary mb-3">자동매매 페이지로 이동합니다</p>
            <Link
        ...
        {subTab === 'portfolio' && (
          <div className="text-center py-8">
            <Briefcase className="w-12 h-12 mx-auto mb-3 text-at-text-weak opacity-30" />
            <p className="text-sm text-at-text-secondary">포트폴리오 기능은 준비 중입니다</p>
          </div>
        )}
      </div>
    </div>
    ...
    return (
      <div className="max-w-lg mx-auto mt-8">
        <div className="bg-at-surface rounded-2xl shadow-at-card p-8 text-center">
          <Link2 className="w-16 h-16 mx-auto mb-4 text-at-accent opacity-50" />
            ...
            <div className="space-y-3">
              {activeStrategies.map(s => (
                <div key={s.id} className="flex items-center justify-between p-3 rounded-xl bg-at-bg">
                  ...
                  <span className="px-2 py-0.5 text-xs rounded-full bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 font-medium">활성</span>
                </div>
              ))}
            </div>
          )}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link
          ...
          className="inline-flex items-center gap-2 px-4 py-2 bg-at-accent text-white rounded-xl text-sm font-medium hover:bg-at-accent-hover transition-colors"
        >
          <Plus className="w-4 h-4" /> 새 전략 만들기
        </Link>
      </div>
            ...
            <div key={s.id} className="bg-at-surface rounded-2xl shadow-at-card p-5">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`px-2 py-0.5 text-xs rounded-full font-medium ${
                      s.is_active ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400'
                ...
                <div className="flex items-center gap-1.5">
                  <Link href={`/investment/strategy/${s.id}/backtest`} className="p-2 rounded-lg hover:bg-at-bg transition-colors text-at-text-secondary" title="백테스트">
                    <BarChart3 className="w-4 h-4" />
                  </Link>
                  <button onClick={() => toggleActive(s.id, s.is_active)} className="p-2 rounded-lg hover:bg-at-bg transition-colors text-at-text-secondary">
                    {s.is_active ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
                  </button>
                  <button onClick={() => deleteStrategy(s.id)} className="p-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors text-red-400" disabled={s.is_active}>
  ...
  return (
    <div className="bg-at-surface rounded-2xl shadow-at-card p-4">
      ...
      <div className="flex items-baseline gap-1">
        <span className="text-2xl font-bold text-at-text">{value}</span>
