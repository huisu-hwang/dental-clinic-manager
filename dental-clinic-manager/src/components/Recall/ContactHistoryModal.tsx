      ...
      <div className="bg-white rounded-2xl shadow-at-card w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-4 border-b border-at-border">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-at-surface-alt flex items-center justify-center">
              <Clock className="w-5 h-5 text-at-text-secondary" />
            </div>
            <div>
              <h3 className="font-semibold text-at-text">연락 이력</h3>
        ...
        <div className="p-4 border-b border-at-border">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-full bg-at-surface-alt flex items-center justify-center">
            ...
            <div className="ml-auto text-right">
              ...
              <p className="text-xs text-at-text-weak mt-1">총 {patient.contact_count}회 연락</p>
            </div>
          </div>

          {/* 예약 정보 */}
          {patient.appointment_date && (
            <div className="mt-3 p-3 bg-at-success-bg rounded-lg flex items-center gap-2">
              <Calendar className="w-5 h-5 text-at-success" />
              <div>
        ...
        <div className="p-4">
          {isLoading ? (
            <div className="py-8 text-center">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-at-accent mx-auto mb-2"></div>
              <p className="text-sm text-at-text-weak">로딩 중...</p>
            </div>
          ) : error ? (
            <div className="py-8 text-center text-at-error">
              <p>{error}</p>
            </div>
          ) : logs.length === 0 ? (
            <div className="py-8 text-center">
              <Clock className="w-12 h-12 text-at-text-weak mx-auto mb-2" />
              <p className="text-at-text-weak">연락 이력이 없습니다.</p>
            </div>
          ) : (
            <div className="space-y-4">
              {logs.map((log) => (
                <div
                  key={log.id}
                  className="relative pl-8 pb-4 border-l-2 border-at-border last:border-l-0 last:pb-0"
                >
                  {/* 타임라인 점 */}
                  <div className={`absolute left-[-9px] top-0 w-4 h-4 rounded-full ${
                    log.contact_type === 'call' ? 'bg-green-500' : 'bg-at-accent'
                  }`}>
                  ...
                  <div className="bg-at-surface-alt rounded-xl p-3">
                    <div className="flex items-center justify-between mb-2">
                      <span className={`inline-flex items-center gap-1 text-xs font-medium ${
                        log.contact_type === 'call' ? 'text-at-success' : 'text-at-accent'
                      }`}>
                        {log.contact_type === 'call' ? (
                    ...
                    {/* 문자 내용 */}
                    {log.sms_content && (
                      <div className="mt-2 p-2 bg-white rounded-lg border border-at-border text-sm text-at-text-secondary">
                        {log.sms_content}
                      </div>
                    )}
        ...
        {/* 푸터 */}
        <div className="flex justify-end p-4 border-t border-at-border">
          <button
            onClick={onClose}
