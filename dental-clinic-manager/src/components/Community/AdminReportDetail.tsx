...
export default function AdminReportDetail({ report, onBack }: AdminReportDetailProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-at-border p-4 sm:p-6 space-y-4 shadow-at-card">
        <h2 className="text-lg font-semibold text-at-text">신고 상세</h2>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            <span className="text-at-text-secondary">사유:</span>
            <span className="ml-2 font-medium">{REPORT_REASON_LABELS[report.reason]}</span>
          </div>
          <div>
            <span className="text-at-text-secondary">신고일:</span>
            <span className="ml-2">{formatDate(report.created_at)}</span>
          </div>
          <div>
            <span className="text-at-text-secondary">유형:</span>
            <span className="ml-2">{report.post_id ? '게시글' : '댓글'}</span>
          </div>
        </div>

        {report.detail && (
          <div className="bg-at-surface-alt rounded-xl p-3">
            <span className="text-xs text-at-text-secondary">추가 설명:</span>
            <p className="text-sm text-at-text-secondary mt-1">{report.detail}</p>
          </div>
        )}

        {/* 신고 대상 콘텐츠 */}
        <div className="border border-at-border rounded-xl p-4">
          <h3 className="text-sm font-semibold text-at-text-secondary mb-2">신고 대상 콘텐츠</h3>
          {report.post && (
            <div>
              <p className="font-medium text-at-text">{report.post.title}</p>
              <div className="text-sm text-at-text-secondary mt-1 max-h-40 overflow-y-auto" dangerouslySetInnerHTML={{ __html: sanitizeHtml(report.post.content) }} />
            </div>
          )}
          {report.comment && (
            <p className="text-sm text-at-text-secondary">{report.comment.content}</p>
          )}
        </div>

        {/* 실명 확인 */}
        {targetProfileId && (
          <div className="border border-at-border rounded-xl p-4 bg-at-warning-bg">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Eye className="w-4 h-4 text-at-warning" />
                <span className="text-sm font-medium text-at-warning">대상자 실명 확인</span>
              </div>
              {!identity ? (
                ...
                <div className="text-sm text-at-warning">
                  <span className="font-medium">{identity.name}</span>
                  <span className="ml-2 opacity-80">({identity.email})</span>
                  <span className="ml-2 opacity-60">{identity.role}</span>
                </div>
              )}
            </div>
          </div>
        ...
        {report.status === 'pending' && !showPenalty && (
          <div className="flex gap-3 pt-2">
            ...
            <Button onClick={handleActionTaken} disabled={processing} className="flex-1 bg-at-error hover:bg-at-error">
...
function AdminPenaltyFormInline({ profileId, onComplete }: { profileId: string; onComplete: () => void }) {
  ...
  return (
    <div className="border border-at-border rounded-xl p-4 bg-at-error-bg space-y-3">
      <h4 className="text-sm font-semibold text-at-error flex items-center gap-2">
        <UserX className="w-4 h-4" />제재 발급
      </h4>
      <select
        value={type}
        onChange={(e) => setType(e.target.value as any)}
        className="w-full border border-at-border rounded-xl px-3 py-2 text-sm"
      >
      ...
      {type === 'temp_ban' && (
        <select value={days} onChange={(e) => setDays(Number(e.target.value))} className="w-full border border-at-border rounded-xl px-3 py-2 text-sm">
      ...
      <textarea
        ...
        className="w-full border border-at-border rounded-xl px-3 py-2 text-sm"
      />
      <Button onClick={handleSubmit} disabled={!reason.trim() || submitting} className="w-full bg-at-error hover:bg-at-error">
