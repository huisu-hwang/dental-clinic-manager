...
const NARRATIVE_REPAIR_SYSTEM_PROMPT = `당신은 주식 시장 군중심리 분석 전문가입니다. 이미 계산된 구조화 분석 결과를 바탕으로, 해석 본문(narrative)만 한국어로 작성합니다.

반드시 지킬 규칙:
- 일반 투자자가 읽는 설명문 3~5문장만 출력합니다.
- JSON, 마크다운 코드블록, 머리말, 항목 리스트를 출력하지 않습니다.
- 추측보다 입력 데이터에서 직접 읽히는 패턴 위주로 설명합니다.
- 매수/매도 추천, 목표가, 손절가 같은 직접적 매매 지시는 금지합니다.
- 특징적 시각을 언급할 때는 반드시 "HH:MM (공포·탐욕 지수 NN, 라벨)" 형식으로 붙입니다.
- markers 로 주어진 시각은 본문에 모두 한 번 이상 반영합니다.
- narrative 본문만 출력합니다.`
...
function buildNarrativeRepairPrompt(
  args: AnalyzeArgs,
  structured: z.infer<typeof ZAnalysis> & { score_label: ScoreLabel; markers: z.infer<typeof ZMarker>[] },
  fearGreed: number[],
): string {
  const recentCandles = args.snapshot.candles
  const markerIndexes = new Set(structured.markers.map((m) => m.candle_index))
  const sampledIndexes = new Set<number>([
    0,
    Math.max(0, recentCandles.length - 1),
    Math.max(0, recentCandles.length - 2),
    Math.max(0, Math.floor(recentCandles.length / 2)),
    ...Array.from(markerIndexes),
  ])
  const sampledLines = Array.from(sampledIndexes)
    .filter((idx) => idx >= 0 && idx < recentCandles.length)
    .sort((a, b) => a - b)
    .map((idx) => {
      const c = recentCandles[idx]
      const kst = toKstParts(c.ts)
      const fg = fearGreed[idx]
      const fgSuffix = typeof fg === 'number' && !Number.isNaN(fg)
        ? ` F=${Math.round(fg)}(${fearGreedLabel(fg)})`
        : ''
      return `[${idx}] ${kst?.iso ?? c.ts} O=${c.open} H=${c.high} L=${c.low} C=${c.close} V=${fmtVolume(c.volume)}${fgSuffix}`
    })

  const markerLines = structured.markers.length > 0
    ? structured.markers.map((marker) => {
        const sourceTs = marker.ts || recentCandles[marker.candle_index]?.ts || ''
        const kst = toKstParts(sourceTs)
        const score = fearGreed[marker.candle_index]
        const scorePart = typeof score === 'number' && !Number.isNaN(score)
          ? `, 공포·탐욕 지수 ${Math.round(score)} (${fearGreedLabel(score)})`
          : ''
        return `- [${marker.candle_index}] ${kst?.hm ?? sourceTs}: ${marker.label} (${marker.kind}${scorePart})`
      }).join('\n')
    : '없음'

  const orderbookLine = structured.orderbook_pressure
    ? `호가 압력: 매수 ${Math.round(structured.orderbook_pressure.bid_pct)}%, 매도 ${Math.round(structured.orderbook_pressure.ask_pct)}%, 해석=${structured.orderbook_pressure.interpretation || '없음'}`
    : '호가 압력: 데이터 없음'

  return [
    `종목: ${args.ticker} (${args.market})`,
    args.asOf ? `분석 시점: ${toKstParts(args.asOf)?.iso ?? args.asOf}` : '분석 시점: 현재',
    `점수: ${structured.psychology_score} (${structured.score_label})`,
    `태그: ${structured.tags.join(', ') || '없음'}`,
    orderbookLine,
    '주요 markers:',
    markerLines,
    '참고 분봉 샘플:',
    sampledLines.join('\n'),
    '',
    '위 정보를 바탕으로 narrative 본문만 작성하세요.',
  ].join('\n')
}

async function generateNarrativeRepair(
  client: Anthropic,
  args: AnalyzeArgs,
  structured: z.infer<typeof ZAnalysis> & { score_label: ScoreLabel; markers: z.infer<typeof ZMarker>[] },
  fearGreed: number[],
): Promise<string> {
  const repairPrompt = buildNarrativeRepairPrompt(args, structured, fearGreed)
  const response = await client.messages.create({
    model: MODEL_ID,
    max_tokens: 700,
    temperature: 0.2,
    system: NARRATIVE_REPAIR_SYSTEM_PROMPT,
...
export async function analyzePsychology(args: AnalyzeArgs): Promise<AnalyzeResult> {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    const normalized = {
      ...parsed.data,
      score_label: validLabel,
      markers: cleanMarkers,
    }

    // tool_use 안의 긴 자유서술(narrative)은 Haiku가 비워서 보내는 경우가 있어,
    // 구조화 결과는 유지하고 narrative만 별도 텍스트 생성으로 복구한다.
    let narrativeTrim = normalized.narrative.trim()
    if (narrativeTrim.length < MIN_NARRATIVE_LENGTH) {
      console.warn('[psychology llm] weak narrative from tool_use, repairing with text generation', {
        ticker: args.ticker,
        market: args.market,
        score: normalized.psychology_score,
        length: narrativeTrim.length,
        markers: cleanMarkers.length,
      })
      try {
        const repaired = await generateNarrativeRepair(client, args, normalized, fearGreedSeries)
        if (repaired.trim().length >= MIN_NARRATIVE_LENGTH) {
          narrativeTrim = repaired.trim()
        }
      } catch (repairError) {
        console.warn('[psychology llm] narrative repair failed', repairError)
      }
    }
    if (narrativeTrim.length < MIN_NARRATIVE_LENGTH) {
      console.warn('[psychology llm] narrative unavailable after repair — using fallback', {
        ticker: args.ticker,
        market: args.market,
        score: normalized.psychology_score,
        length: narrativeTrim.length,
    ...
    const baseNarrative = narrativeTrim ||
      `심리 점수 ${normalized.psychology_score} (${validLabel}). 모델이 상세 해설을 생성하지 못해 기본 정보만 표시합니다.`
    ...
    const enriched = enrichNarrativeWithFearGreed(
      { ...normalized, narrative: baseNarrative },
      fearGreedSeries,
