...
function buildBrandDesignPrefix(ctx: BrandPromptCtx | null, imageStyle?: ImageStyleOption): string {
  ...
  if (isInfographic) {
    ...
    if (primary) {
      lines.push(`4) 하단(전체 높이의 60~80% 위치): 주 브랜드 컬러(${primary}) 배경 박스 안에 화이트 체크마크(✓) + 핵심 포인트 한 줄 (불릿 1개).`);
    } else {
    ...
    lines.push('  · 영문 헤더(클리닉명)는 위 글자수에서 제외 — 헤더는 항상 짧게 유지. 한글 푸터는 그리지 마세요(별도 합성).');
