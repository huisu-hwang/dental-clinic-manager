...
import {
  ...
  BarChart3, Award, Clock, Plus, X, Trophy,
} from 'lucide-react'
  ...
  const runAllBacktests = async () => {
    ...
    const promises = tickers.map(async (entry) => {
      try {
        ...
        const json = await res.json()
        if (!res.ok) {
          return { ticker: entry.ticker, error: json.error }
        }
    ...
    for (const result of settled) {
      ...
      } else if ('metrics' in result && result.metrics) {
        successes.push(result as BacktestResultData)
      }
    }
      ...
      <div className="bg-at-surface rounded-2xl shadow-at-card p-5 space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <TickerSearch
            ...
            onChange={(ticker, name) => {
              if (ticker) addTicker(ticker, name)
            }}
            market={addingMarket}
            className="flex-1"
          />
        ...
        {tickers.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {tickers.map((t, i) => (
              <span
                key={`${t.ticker}-${t.market}-${i}`}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-at-bg text-sm"
              >
                <span className={`px-1 py-0.5 rounded text-[10px] font-bold ${
                  t.market === 'KR' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400'
                ...
                {t.name !== t.ticker && (
                  <span className="text-at-text-secondary text-xs">{t.name}</span>
                )}
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {[
            ...
            <button
              ...
              onClick={() => {
                if (!tickers.some(t => t.ticker === q.ticker && t.market === q.market)) {
                  setTickers(prev => [...prev, q])
                }
              }}
        ...
        >
          {isRunning ? (
            <>
              ...
              {tickers.length}개 종목 백테스트 중... ({tickers.length - runningTickers.size}/{tickers.length})
            </>
          ) : (
            <>
              <Play className="w-4 h-4" />
              {tickers.length}개 종목 백테스트 실행
            </>
          )}
        </button>
              ...
              <tbody>
                {results.map((r, i) => (
                  <tr
                    ...
                    className={`border-b border-at-border/50 cursor-pointer transition-colors ${
                      selectedResultIdx === i
                        ? 'bg-at-accent-light/50'
                        : 'hover:bg-at-bg'
                    }`}
                  >
                    <td className="py-2.5 px-2">
                      {i === 0 ? <Trophy className="w-3.5 h-3.5 text-amber-500" /> :
                       <span className="text-at-text-weak">{i + 1}</span>}
                    </td>
                    <td className="py-2.5 px-2">
                      <span className="font-mono font-semibold text-at-text">{r.ticker}</span>
                      {r.tickerName && r.tickerName !== r.ticker && (
                        <span className="text-at-text-secondary ml-1">{r.tickerName}</span>
                      )}
                    </td>
                    <td className="py-2.5 px-2 text-center">
                      <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${
                        r.market === 'KR' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400'
                          : 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400'
                      }`}>{r.market}</span>
                    </td>
                    <td className={`py-2.5 px-2 text-right font-mono font-semibold ${
                      r.metrics.totalReturn >= 0 ? 'text-red-500' : 'text-blue-500'
                    }`}>
...
function MetricCard({ label, value, positive, icon }: {
  ...
  return (
    <div className="bg-at-surface rounded-2xl shadow-at-card p-4">
      ...
      <p className={`text-lg font-bold font-mono ${positive ? 'text-red-500' : 'text-blue-500'}`}>
        {value}
      </p>
    </div>
  )
