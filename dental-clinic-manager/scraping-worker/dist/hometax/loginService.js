...
async function performLogin(page, loginId, loginPw, residentNumber) {
    try {
        ...
        const jsIdTabResult = await page.evaluate(() => {
            ...
            while (node) {
                ...
                const directText = Array.from(el.childNodes)
                    .filter((n) => n.nodeType === win.Node.TEXT_NODE)
        ...
        const btnHtml = await page.evaluate(() => {
            /* eslint-disable @typescript-eslint/no-explicit-any */
            const doc = globalThis.document;
            const buttons = Array.from(doc.querySelectorAll('a, button, input[type="submit"], input[type="button"], input[type="password"], input[type="text"]'))
                .filter((el) => {
                ...
                .map((el) => `<${el.tagName} id="${el.id}" class="${el.className}" type="${el.type || ''}">${el.textContent?.trim().substring(0, 30)}</${el.tagName}>`)
                .join('\n');
            return buttons;
        }).catch(() => '');
        ...
        if (has2FA) {
            ...
            const popupInfo = await page.evaluate(() => {
                ...
                if (!popup)
                    return { found: false, inputs: [], buttons: [] };
                const inputs = Array.from(popup.querySelectorAll('input')).map((el) => ({
                    id: el.id, type: el.type, class: el.className, placeholder: el.placeholder || '', visible: el.offsetParent !== null || el.offsetWidth > 0,
                }));
                const buttons = Array.from(popup.querySelectorAll('a, button, input[type="button"], input[type="submit"]')).map((el) => ({
            ...
            if (!genderTyped) {
                ...
                await page.evaluate((val) => {
                    ...
                    const inputs = Array.from(popup.querySelectorAll('input:not([type="hidden"])'))
                ...
                }, gender1).catch((e) => log.warn({ err: e }, '2차 인증 성별 JS 폴백 실패'));
            }
            ...
            if (!confirmClicked) {
                ...
                }).catch((e) => log.warn({ err: e }, '2차 인증 확인 버튼 JS 폴백 실패'));
            }
            await page.waitForTimeout(3000);
