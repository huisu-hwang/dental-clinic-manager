 ...
 *   2.5 필요 데이터 스크래핑
 */
export async function scrapeCashReceiptSales(context, year, month, _clinicId, sharedPage) {
    log.info({ year, month }, '현금영수증 매출내역 누계조회 스크래핑 시작');
    const doScrape = async (page) => {
        if (!sharedPage) {
        ...
        const navResult = await page.evaluate((menuId) => {
        ...
        const formElements = await page.evaluate(() => {
            ...
            return Array.from(doc.querySelectorAll('input, select'))
        ...
        const data = await page.evaluate(() => {
            ...
            for (const table of Array.from(tables)) {
                if (table.offsetParent === null && table.offsetWidth === 0)
                    continue;
                const headers = [];
        ...
        if (data.length === 0) {
            const bodyText = await page.evaluate(() => (globalThis.document.body?.innerText || '').substring(0, 1500)).catch(() => '');
 ...
 *   2.6 필요정보 스크래핑
 */
export async function scrapeCashReceiptPurchase(context, year, month, _clinicId, sharedPage) {
    ...
    const doScrape = async (page) => {
        ...
        const formElements = await page.evaluate(() => {
            /* eslint-disable @typescript-eslint/no-explicit-any */
            const doc = globalThis.document;
            return Array.from(doc.querySelectorAll('input, select, label, a.w2anchor2, button'))
                .filter((el) => el.offsetParent !== null || el.offsetWidth > 0)
                .map((el) => ({
                tag: el.tagName, id: el.id?.substring(0, 60) || '', type: el.type || '',
        ...
        const periodSet = await page.evaluate((p) => {
            /* eslint-disable @typescript-eslint/no-explicit-any */
            const doc = globalThis.document;
            const win = globalThis;
            const results = [];
            ...
            if (dateInputs.length >= 2) {
                ...
                // 시작일
                if (setter)
                    setter.call(dateInputs[0], p.start);
                else
                    dateInputs[0].value = p.start;
                dateInputs[0].dispatchEvent(new win.Event('change', { bubbles: true }));
                results.push(`start: ${dateInputs[0].id} → ${p.start}`);
                // 종료일
                if (setter)
                    setter.call(dateInputs[1], p.end);
                else
                    dateInputs[1].value = p.end;
