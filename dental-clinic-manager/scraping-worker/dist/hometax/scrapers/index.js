 ...
 */
export async function runScraper(context, dataType, year, month, clinicId) {
    ...
    return scraper(context, year, month, clinicId);
}
/**
 * 공유 페이지로 스크래퍼 실행 (Playwright 모드 전용)
 * 이미 로드된 메인 페이지가 있을 때 goto/로그인 확인 단계를 건너뜁니다.
 */
export async function runScraperWithPage(page, session, dataType, year, month, clinicId) {
