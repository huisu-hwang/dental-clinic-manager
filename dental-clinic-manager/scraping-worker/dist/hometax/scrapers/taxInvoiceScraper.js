import { withPage } from './baseScraper.js';
...
// ── 공통 헬퍼 ──
/** JS 기반 안전한 클릭 */
async function jsClick(page, selector, description, timeout = 10000) {
    try {
...
async function clickByText(page, text, description, opts) {
    const result = await page.evaluate(({ text: t, exact, parentSel }) => {
        /* eslint-disable @typescript-eslint/no-explicit-any */
        const doc = globalThis.document;
        const root = parentSel ? doc.querySelector(parentSel) || doc.body : doc.body;
        const walker = doc.createTreeWalker(root, globalThis.NodeFilter.SHOW_ELEMENT);
...
/** 인증서 비밀번호 조회 */
async function getCertPassword(clinicId) {
    ...
    const { data } = await supabase
        ...
        .single();
    if (!data?.encrypted_cert_password)
        return null;
    try {
 ...
 *   2.9 데이터 스크래핑
 */
async function navigateToTaxInvoiceStats(page, clinicId) {
    ...
    if (hasCertPopup) {
        ...
        const certPopupInfo = await page.evaluate(() => {
            ...
            if (!popup)
                return { found: false, text: '', inputs: [], buttons: [] };
            return {
                found: true,
                text: popup.textContent?.substring(0, 500) || '',
                inputs: Array.from(popup.querySelectorAll('input')).map((el) => ({
                    id: el.id, type: el.type, class: el.className, placeholder: el.placeholder || '',
                })),
                buttons: Array.from(popup.querySelectorAll('a, button, input[type="button"]')).map((el) => ({
        ...
        if (certPassword) {
            ...
            const pwTyped = await page.evaluate((pw) => {
                /* eslint-disable @typescript-eslint/no-explicit-any */
                const doc = globalThis.document;
                const win = globalThis;
                // 팝업 내 비밀번호 필드
                const pwFields = doc.querySelectorAll('input[type="password"]');
                for (const field of Array.from(pwFields)) {
...
async function searchByYear(page, year) {
    ...
    const yearSet = await page.evaluate((y) => {
        ...
        for (const input of Array.from(yearInputs)) {
...
/** Step 2.9: 데이터 스크래핑 */
async function scrapeStatisticsData(page) {
    ...
    const data = await page.evaluate(() => {
        ...
        if (gridRows.length > 0) {
            ...
            for (const row of Array.from(gridRows)) {
    ...
    if (data.length === 0) {
        ...
        const pageText = await page.evaluate(() => {
            return (globalThis.document.body?.innerText || '').substring(0, 2000);
        }).catch(() => '');
...
export async function scrapeTaxInvoiceSales(context, year, _month, clinicId) {
...
export async function scrapeTaxInvoicePurchase(context, year, _month, clinicId) {
