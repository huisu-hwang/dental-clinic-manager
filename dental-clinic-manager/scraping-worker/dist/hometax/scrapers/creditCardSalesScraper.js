 ...
 * 2.4 필요 정보 스크래핑
 */
export async function scrapeCreditCardSales(context, year, month, _clinicId, sharedPage) {
    // 사용자 설정 월 기준 분기 계산
    ...
    const doScrape = async (page) => {
        ...
        const yearSet = await page.evaluate((y) => {
            ...
            for (const el of Array.from(combos)) {
                if (el.offsetParent === null && el.offsetWidth === 0)
                    continue;
        ...
        // 분기 선택 (오늘 날짜 기준)
        const quarterSet = await page.evaluate((q) => {
            /* eslint-disable @typescript-eslint/no-explicit-any */
            const doc = globalThis.document;
            const win = globalThis;
            const qStr = String(q);
...
//# sourceMappingURL=creditCardSalesScraper.js.map