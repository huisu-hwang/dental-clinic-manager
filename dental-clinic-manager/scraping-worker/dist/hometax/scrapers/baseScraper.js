...
/** 홈택스 메뉴 페이지로 이동 */
export async function navigateToMenu(page, menuPath) {
    ...
    const menuExploreResult = await page.evaluate(() => {
        ...
        const menuItems = Array.from(doc.querySelectorAll('a[id*="menuAtag"], a[id*="combineMenuAtag"]'))
            .map((el) => ({
            id: el.id, text: el.textContent?.trim() || '', onclick: el.getAttribute('onclick') || '',
        }));
        // 조회 관련 메뉴 필터링
        const queryMenus = menuItems.filter((m) => m.text.includes('조회') || m.text.includes('목록') || m.text.includes('합계표') || m.text.includes('현황'));
    ...
    if (!targetMenuId) {
        ...
        for (const keyword of keywords) {
            const match = menuExploreResult.allMenus.find((m) => m.text.includes(keyword) && m.onclick.includes('fn_topMenuOpen'));
            if (match) {
    ...
    if (targetMenuId) {
        ...
        const openResult = await page.evaluate((menuId) => {
            /* eslint-disable @typescript-eslint/no-explicit-any */
...
/** 조회 기간 설정 (연/월) */
export async function setPeriod(page, year, month) {
...
export async function clickSearchAndWait(page) {
    ...
    if (!clicked) {
        // 디버그: 페이지 텍스트와 URL 덤프
        const pageText = await page.evaluate(() => (globalThis.document?.body?.innerText || '').substring(0, 2000)).catch(() => ''); // eslint-disable-line @typescript-eslint/no-explicit-any
...
/** HTML 테이블에서 데이터 추출 */
export async function parseTable(page, tableSelector) {
...
export async function scrapeAllPages(page, tableSelector) {
    const allRecords = [];
    // 첫 페이지 데이터
