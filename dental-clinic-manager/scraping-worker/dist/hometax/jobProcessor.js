...
import { runScraper, runScraperWithPage, getScrapingMode } from './scrapers/index.js';
...
import { getBrowserContext } from '../types/scrapingContext.js';
...
/** 진행 상태 메시지 업데이트 */
async function updateProgress(jobId, message) {
        ...
        if (session.type === 'playwright') {
            ...
            try {
                ...
                for (const dataType of dataTypes) {
                    try {
                        ...
                        const result = await runScraperWithPage(sharedPage, session, dataType, year, month, clinicId);
                        // 결과 저장
            ...
            for (const dataType of dataTypes) {
                try {
                    ...
                    const result = await runScraper(session, dataType, year, month, clinicId);
                    // 결과 저장
                    await saveRawData(clinicId, result);
                    await logSync(clinicId, jobId, dataType, 'success', result.totalCount);
                    results[dataType] = { success: true, count: result.totalCount };
