...
/** 일일 수집 완료 알림 */
export async function notifySyncComplete(clinicId, results) {
...
/** 월말 결산 완료 알림 */
export async function notifySettlementComplete(clinicId, year, month, settlement) {
    await createNotification({
        clinicId,
...
/** 수집 실패 알림 */
export async function notifySyncFailed(clinicId, errorMessage, jobId) {
    await createNotification({
        clinicId,
        type: 'sync_failed',
        title: '홈택스 수집 실패',
