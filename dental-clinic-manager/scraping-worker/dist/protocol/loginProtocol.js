...
export async function loginViaProtocol(clinicId) {
    ...
    if (savedSession?.cookies) {
        ...
        session.cookies = savedSession.cookies;
        // 세션 유효성 검증
        try {
            const verifyRes = await httpGet(session, '/websquare/websquare.wq?w2xPath=/ui/pp/index_pp.xml');
            if (verifyRes.body.includes('로그아웃') || verifyRes.body.includes('마이홈택스')) {
    ...
    try {
        await withRetry(async () => {
            ...
            return result;
        }, 'protocol-login', { maxRetries: 2, delays: [3000, 5000] });
        // 로그인 성공 → 세션 저장
...
/** Protocol 모드 로그아웃 */
export async function logoutViaProtocol(session) {
