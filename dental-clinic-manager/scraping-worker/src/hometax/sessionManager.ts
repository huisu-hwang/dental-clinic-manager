...
/** 현재 컨텍스트 쿠키를 DB에 저장 */
export async function saveSession(clinicId: string, context: BrowserContext): Promise<void> {
  const cookies = await context.cookies();
  const hometaxCookies = cookies.filter(c =>
    c.domain.includes('hometax.go.kr') || c.domain.includes('nts.go.kr')
  );
...
export async function loadSession(clinicId: string): Promise<StoredSession | null> {
  ...
  const session = data.session_data as StoredSession;
