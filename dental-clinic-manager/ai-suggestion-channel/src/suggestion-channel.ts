...
const mcp = new Server(
  { name: 'suggestion-inbox', version: '0.2.0' },
  {
    ...
    instructions: [
      ...
      '3. /Users/hhs/Project/dental-clinic-manager/dental-clinic-manager 에서 CLAUDE.md 원칙대로 작업: develop 브랜치를 베이스로 git checkout -b feat/suggestion-{task_id} origin/develop 으로 분기, 관련 파일을 먼저 읽고 최소 침습적으로 수정, npm run build 통과 확인 후 커밋·푸시·PR 생성. **PR은 반드시 `gh pr create --base develop --head feat/suggestion-{task_id} ...` 로 develop을 베이스로 생성**.',
      ...
      '2. 작업 디렉토리(/Users/hhs/Project/dental-clinic-manager/dental-clinic-manager)에서 git fetch origin develop 후 메시지 안내된 "작업 브랜치"를 git checkout. (브랜치는 이미 develop 기반으로 생성돼 있으므로 그대로 사용. 필요 시 git pull로 최신화)',
