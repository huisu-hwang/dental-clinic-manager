...
const mcp = new Server(
  { name: 'suggestion-inbox', version: '0.2.0' },
  {
    ...
    instructions: [
      ...
      '3. 사용자가 긍정하면 즉시 update_suggestion_task(task_id=<태그의 task_id>, status="running", progress_step="analyzing") 을 호출해 웹앱에 상태 노출.',
      '4. /Users/hhs/Project/dental-clinic-manager/dental-clinic-manager 에서 CLAUDE.md 원칙대로 작업: 관련 파일을 먼저 읽고, 최소 침습적으로 수정, npm run build 통과 확인 후 feat/suggestion-{task_id} 브랜치로 커밋·푸시·PR 생성.',
      '5. 단계 전환마다 update_suggestion_task(task_id, progress_step) 호출을 권장 (editing/building/committing/pushing/creating_pr).',
      '6. PR 생성 성공 시 update_suggestion_task(task_id, status="completed", pr_url=..., pr_number=..., commit_sha=..., worker_log=<한 줄 요약>) 호출.',
      '7. 중간에 구현 불가·빌드 실패·기타 오류로 중단 시 update_suggestion_task(task_id, status="failed", error_message=<사유>) 호출 후 사용자에게 상황 보고.',
      '8. 사용자가 "나중에"/"무시"라고 답하면 update_suggestion_task(task_id, status="cancelled") 호출 후 아무 작업도 수행하지 말고 다음 이벤트를 대기.',
      '',
