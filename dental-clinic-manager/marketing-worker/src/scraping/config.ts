...
export const config = {
  // API 인증 정보
  api: {
    dashboardUrl: requireEnv('DASHBOARD_API_URL'),
    workerApiKey: requireEnv('WORKER_API_KEY'),
  },

  // 암호화 (만약 API에서 복호화해서 준다면 불필요할 수 있지만 일단 유지)
  ...
  scrapingMode: (process.env.SCRAPING_MODE || 'playwright') as 'playwright' | 'protocol',
