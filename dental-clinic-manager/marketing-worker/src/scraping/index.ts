...
export async function startScrapingWorker(): Promise<void> {
  ...
  if (!connected) {
    log.fatal('Supabase 연결 실패, 워커 시작 중단');
    return;
  }
...
export async function stopScrapingWorker(): Promise<void> {
  await shutdown('MANUAL_STOP');
}

// CLI로 직접 실행될 때만 main() 호출
if (import.meta.url === `file://${process.argv[1]}`) {
  startScrapingWorker().catch((err) => {
