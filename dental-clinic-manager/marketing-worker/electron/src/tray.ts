...
import { getDentwebBridgeStatus } from './dentweb-bridge';
...
function rebuildMenu(): void {
  ...
  const dentwebLabels: Record<string, string> = {
    ...
    syncing: '동기화 중',
    error: '오류',
    'db-unavailable': 'DB 연결 불가',
    'not-configured': '미설정',
  };
  const dentwebState = getDentwebBridgeStatus();
  const dentwebLabel = dentwebLabels[dentwebState.status] ?? '알 수 없음';
