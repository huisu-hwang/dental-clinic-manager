 ...
 */
async function downloadImages(generatedImages) {
 ...
 * API 모드: 개별 항목 발행
 */
async function publishItemApi(item) {
    const client = getApiClient();
...
async function publishItemSupabase(item) {
    const sb = await getSupabase();
    ...
    try {
        ...
        try {
            ...
            parsedContent = { title: item.title, body: content };
        }
        const generatedImages = item.generated_images;
        ...
        if (platforms.naverBlog) {
            ...
            const { data: platformSettings } = await sb
                ...
                .single();
            const blogConfig = platformSettings?.config;
            if (!publisher) {
                publisher = new NaverBlogPublisher();
                await publisher.init(blogConfig ? {
                    blogId: blogConfig.blogId || blogConfig.naverId || '',
 ...
 */
export async function processScheduledItemsOnce() {
    try {
        await processScheduledItems();
    }
    catch (error) {
