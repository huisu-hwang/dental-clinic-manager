...
import { humanType, typeBodyContent, typeHashtags, waitForPageStable, handleDraftPopup, } from '../typing-simulator.js';
// ============================================
// 네이버 블로그 자동 발행기
...
export class NaverBlogPublisher {
    ...
    async publish(postData) {
        ...
        try {
            ...
            try {
                await page.screenshot({ path: `/tmp/naver-error-${Date.now()}.png` });
            }
            catch { /* ignore */ }
    ...
    async performLogin(page) {
        ...
        if (this.context) {
            try {
                ...
                const fs = await import('fs');
                const path = await import('path');
                fs.writeFileSync(path.join(import.meta.dirname, '..', 'naver-cookies.json'), JSON.stringify(cookies, null, 2));
     ...
     */
    async typeBody(page, body, images) {
        ...
        await randomDelay(delays.titleToBody);
        let imageIndex = 0;
        await typeBodyContent(page, body, 
        // [FIX #4] 이미지 삽입
        async (_prompt) => {
     ...
     */
    async insertImage(page, imagePath) {
        try {
            const fs = await import('fs');
     ...
     */
    async clickPublish(page, hashtags) {
