-- 기존 dcm.html과 호환되는 하얀치과 실시간 업무 대시보드 데이터베이스 스키마

-- 일일 보고서 종합 테이블 (기존 구조 유지)
